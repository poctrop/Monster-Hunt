...
class Game extends JPanel implements Runnable {// implements runnable allows the use of a thread
    ...
    public float screenY = StartScreen.SCREEN_HEIGHT / 2;
    ...
    private Manager tm = new Manager(8, p); // This class handles the npcs, vechile. Background and tile drawing system
                                            // of the game 
    ...
    public Game() {
        ...
        this.setLayout(null);
        this.setDoubleBuffered(true); // allows for the computer to draw the next image while the current image is
                                      // being displayed
    ...
    @Override
    public void run() { // handles the game loop of the game, runs his simultaniously with the main
                        // mehtod
        long drawInterval = 1000 / 70;// how long in milliseconds the omputer mut wait for the next draw
        while (notHome) {
            if (paused == false) { // if the player has not paused, the game listens for key inputs if it isnt iust
                                   // top listening
    ...
    public void paintComponent(Graphics g) {
        super.paintComponent(g); //uses the features of the JPanel class to draw.
        g.setFont(f);
        ...
        // checks if the player is near the edges of the map or not if the player is
        // near the edge the position on the screen changes and the map stops moving

        if (tm.MAPNUM != 4) {
            if (tm.MAPNUM == 5) {
                if ((p.xPos > 490) && (p.xPos < 2083 - StartScreen.SCREEN_LENGTH - 36)) {
                    p.setxBounds(true);
                } else {
                    p.setxBounds(false);
                }
            ...
            if ((p.yPos > 0) && (p.yPos < 1083 - StartScreen.SCREEN_HEIGHT - 24)) {
                p.setyBounds(true);
            } else {
                p.setyBounds(false);
            }
            ...
            // if the player is not on the edge of the screen the player is drawn in the
            // middle of the screen, if the player is on the edges of the screen
            // then the players screen position will change with its x position
            if (p.isxBounds() == true) {
                if (p.isyBounds()) {
                    g2.drawImage(p.getSprite1(), StartScreen.SCREEN_LENGTH / 2, StartScreen.SCREEN_HEIGHT / 2, 120, 120,
                            null);
                ...
                if (p.isyBounds()) {
                    g2.drawImage(p.getSprite1(), (int) p.getScreenX(), StartScreen.SCREEN_HEIGHT / 2, 120, 120, null);
                } else if (p.isyBounds() == false) {
            ...
            if ((p.yPos > 0) && (p.yPos < 48 * 50 - StartScreen.SCREEN_HEIGHT - 24)) {
                p.setyBounds(true);
            } else {
                p.setyBounds(false);
            }
        ...
        if (p.getObjList().size() != 0) {
            for(int i = 0; i < p.getObjList().size(); i++){
                g2.drawImage(p.getObjList().get(i).getBi(), StartScreen.SCREEN_LENGTH, 48*i, 48, 48, null);
            }       
        }
