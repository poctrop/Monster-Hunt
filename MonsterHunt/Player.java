...
class Player extends Entities {
    ...
    public static Music jump, walk, sound;
    ...
    public Player(Keys k) {
        ...
        yPos = startPosY;
        try {
            jump = new Music(MonsterHunt.WORK_DIR + "\\Music\\jump.wav");
            sound = new Music(MonsterHunt.WORK_DIR + "\\Music\\gameMusic2.wav");
            sound.loop();
    ...
    public void isGround() {
        if (tm.getMapNum() == 1) {
            ...
            if ((int) yPos > 260 - speedDown && (int) yPos < 260 && xPos < 1110 && playerJump - speedDown <= 0) {
                onPlatform = true;
                touchGround = false;
                yPos = 260;
                level = true;
                ...
                onPlatform = false;
                touchGround = true;
            } else if ((int) yPos == 260 && (int) xPos < 1110) {
                onPlatform = true;
                touchGround = false;
                speedDown = 0;
                // checks if the player is on the ground
    ...
    public void update(Graphics2D g) {
        if (tm.getMapNum() != 4) {
            ...
            if (xBounds && yBounds) { // checks whether the player is near the edges of the screen
                ...
                screenY = StartScreen.SCREEN_HEIGHT / 2;
            ...
            //switching between the different backgrounds
            ...
            if (xBounds && yBounds) { // checks whether the player is near the edges of the screen
                ...
                if (k.isDownP() && yPos < 48 * 50 - StartScreen.SCREEN_LENGTH / 2) {
                    ...
                    xPos -= driveSpeed;
                } else if (k.isUpP() && yPos > -StartScreen.SCREEN_HEIGHT / 2) {
                    screenY -= driveSpeed;
                    yPos -= driveSpeed;
                } else if (k.isRightP()) {
                    xPos += driveSpeed;
                ...
                if (k.isDownP() && yPos < 48 * 50 - StartScreen.SCREEN_HEIGHT / 2) {
                    yPos += driveSpeed;
                    screenY += driveSpeed;
                } else if (k.isLeftP() && xPos > -StartScreen.SCREEN_LENGTH / 2) {
                    screenX -= driveSpeed;
        ...
        if (tm.MAPNUM == 1 && xPos > 1150 && xPos < 1150 + 200 && yPos > 600) {
            if (k.isInteract() && findObject("keys") != null) {
                tm.setMapNum(5);
                k.setInteract(false);
            }else if(k.isInteract() && findObject("keys") == null){
                g.drawString("You need keys to enter the house", StartScreen.SCREEN_LENGTH/2, StartScreen.SCREEN_HEIGHT/2);
            }
        }
        if (tm.MAPNUM == 5 && xPos > 1150 && xPos < 1150 + 200 && yPos > 600) {
            if (k.isInteract()) {
                tm.setMapNum(1);
                k.setInteract(false);
            }
